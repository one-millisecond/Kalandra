...
void add_token(TokenTable *table, int line, int token_index, const char *token_value) {
    if (line >= table->num_lines || token_index >= table->max_tokens_per_line) {
        printf("Error: Index out of bounds (line: %d, token_index: %d)\n", line, token_index);
        return;
    }
    strncpy(table->table[line][token_index].value, token_value, MAX_TOKEN_SIZE);
}
void remove_token(TokenTable *table, int line, int token_index) {
    if (line >= table->num_lines || token_index >= table->max_tokens_per_line) {
